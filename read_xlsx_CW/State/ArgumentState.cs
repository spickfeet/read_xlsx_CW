...
namespace CW_read_xlsx.State
{
    class ArgumentState : IState
    {
        public void Enter(StateMachine sm)
        {
            ...
            if (sm.TokensData[sm.CurrentTokenIndex].Token == TokenEnum.COMMA)
            {
                sm.State = new CommaState();
            }
            else if (sm.TokensData[sm.CurrentTokenIndex].Token == TokenEnum.CLOSE_BRACKET)
            {
            ...
            else
            {
                sm.ErrorsData.Add(new(sm.TokensData[sm.CurrentTokenIndex].Line,
                    ...
                    $"<{sm.TokensData[sm.CurrentTokenIndex].TokenValue}> не является ожидаемым, ожидается <,> или <)>.",
